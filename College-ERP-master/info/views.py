...
from .models import Dept, Class, Student, Attendance, Course, Teacher, Assign, AttendanceTotal, time_slots, \
    DAYS_OF_WEEK, AssignTime, AttendanceClass, StudentCourse, Marks, MarksClass, Office_staff
from django.urls import reverse
from django.utils import timezone
from django.contrib.auth.decorators import login_required
from django.contrib.auth import get_user_model
from info.forms import Office_staff_form
...
def office_staff(request):
    ...
    data = Office_staff.objects.all()
    print(data)
    return render(request, 'info/office_staff.html', {'data' : data})

def add_office_staff(request):
    form = Office_staff_form()
    
    if request.method == 'POST':
        form = Office_staff_form(request.POST)
        if form.is_valid():
            form.save()
    ...
    return render(request, 'info/add_office_staff.html', context)

def update_officestaff(request, ID):

    update = Office_staff.objects.get(ID=ID)
    form = Office_staff_form(instance=update)

    if request.method == 'POST':
        form = Office_staff_form(request.POST, instance=update)
        if form.is_valid():
            form.save()
            return redirect('office_staff')
    ...
    return render(request, 'info/update_office_staff.html', context)
...
def delete_officestaff(request, ID):
    ...
    context = {
        'office_staff': delete_staff,
    }

    return render(request,'info/office_staff.html', context)